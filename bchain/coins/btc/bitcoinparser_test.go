...
func TestGetAddrDescFromAddress(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args:    args{address: strings.Repeat("1", 257)},
			wantErr: true,
		},
	}
...
func TestGetAddrDescFromAddressRejectsOversizedInput(t *testing.T) {
	parser := NewBitcoinParser(GetChainParams("main"), &Configuration{})
	_, err := parser.GetAddrDescFromAddress(strings.Repeat("1", 257))
	if err == nil || err.Error() != "Address exceeds the 256-byte limit" {
		t.Fatalf("oversized address error = %v, want bounded-input error", err)
	}
}
