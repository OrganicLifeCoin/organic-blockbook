...
const xpubLockStripes = 64

var xpubLocks [xpubLockStripes]sync.Mutex

func lockXpub(xpub string) func() {
	hash := uint32(2166136261)
	for i := 0; i < len(xpub); i++ {
		hash ^= uint32(xpub[i])
		hash *= 16777619
	}
	lock := &xpubLocks[hash%xpubLockStripes]
	lock.Lock()
	return lock.Unlock
}
