...
func TestHTMLTemplateHandlerReturnsAfterRedirect(t *testing.T) {
	s := &PublicServer{}
	handler := s.htmlTemplateHandler(func(w http.ResponseWriter, r *http.Request) (tpl, *TemplateData, error) {
		http.Redirect(w, r, "/block/example", http.StatusFound)
		return noTpl, nil, nil
	})

	w := httptest.NewRecorder()
	handler(w, httptest.NewRequest(http.MethodGet, "/search?q=example", nil))

	if w.Code != http.StatusFound {
		t.Fatalf("redirect status = %d, want %d", w.Code, http.StatusFound)
	}
	if location := w.Header().Get("Location"); location != "/block/example" {
		t.Fatalf("redirect location = %q, want %q", location, "/block/example")
	}
}
	...
	tests := []struct {
		...
		},
		{
			...
			body: []string{
				...
				`<div class="status-banner status-banner-warning">Synchronization with backend is disabled, the state of index is not up to date.</div>`,
		...
		},
		{
			...
			body: []string{
				...
				`<textarea class="form-control" rows="8" name="hex" placeholder="Paste a signed raw transaction hex string"></textarea>`,
				`</html>`,
			},
		},
		{
			...
			body: []string{
				`<a class="nav-link" href="/blocks">Blocks</a>`,
				`<h1>Send Raw Transaction</h1>`,
				`<textarea class="form-control" rows="8" name="hex" placeholder="Paste a signed raw transaction hex string">12341234</textarea>`,
				`<div class="alert alert-danger">Invalid data</div>`,
				`</html>`,
			},
		},
		{
			name:        "explorerSendTx oversized transaction",
			r:           newPostFormRequest(ts.URL+"/sendtx", "hex", strings.Repeat("a", maxSignedTransactionHexBytes+1)),
			status:      http.StatusOK,
			contentType: "text/html; charset=utf-8",
			body: []string{
				`<div class="alert alert-danger">signed transaction exceeds the 4 MiB limit</div>`,
			},
		},
		{
			name:        "explorerSendTx oversized form",
			r:           newPostFormRequest(ts.URL+"/sendtx", "hex", strings.Repeat("a", maxSignedTransactionHexBytes+(2<<10))),
			status:      http.StatusOK,
			contentType: "text/html; charset=utf-8",
			body: []string{
				`<div class="alert alert-danger">Invalid transaction form</div>`,
			},
		},
		{
			name:        "apiIndex",
		...
		{
			name:        "apiAddress v2 oversized address",
			r:           newGetRequest(ts.URL + "/api/v2/address/" + strings.Repeat("1", 257)),
			status:      http.StatusBadRequest,
			contentType: "application/json; charset=utf-8",
			body: []string{
				`{"error":"Invalid address, Address exceeds the 256-byte limit"}`,
			},
		},
		{
		...
		},
		{
			...
			contentType: "application/json; charset=utf-8",
			body: []string{
				`[{"txid":"7c3be24063f268aaa1ed81b64776798f56088757641a34fb156c4f51ed2e9d25","vout":1,"value":"917283951061","height":225494,"confirmations":1,"stake_contract":false}]`,
			},
		},
		{
			...
			contentType: "application/json; charset=utf-8",
			body: []string{
				`[{"txid":"3d90d15ed026dc45e19ffb52875ed18fa9e8012ad123d7f7212176e2b0ebdb71","vout":0,"value":"118641975500","height":225494,"confirmations":1,"address":"2N6utyMZfPNUb1Bk8oz7p2JqJrXkq83gegu","path":"m/49'/1'/33'/1/3","stake_contract":false}]`,
			},
		},
		{
			name:        "apiSendTx",
...
func websocketTestsBitcoinType(t *testing.T, ts *httptest.Server) {
	...
	tests := []struct {
		...
		},
		{
			...
			want: `{"id":"4","data":[{"txid":"00b2c06055e5e90e9c82bd4181fde310104391a7fa4f289b1704e5d90caa3840","vout":0,"value":"100000000","height":225493,"confirmations":2,"stake_contract":false}]}`,
		},
		{
			name: "websocket getAccountUtxo",
			req: websocketReq{
		...
		{
			name: "websocket oversized account descriptor",
			req: websocketReq{
				Method: "getAccountInfo",
				Params: map[string]interface{}{
					"descriptor": strings.Repeat("1", 257),
				},
			},
			want: `{"id":"15","data":{"error":{"message":"Invalid address, Address exceeds the 256-byte limit"}}}`,
		},
		{
			name: "websocket oversized address subscription",
			req: websocketReq{
				Method: "subscribeAddresses",
				Params: map[string]interface{}{
					"addresses": []string{strings.Repeat("1", 257)},
				},
			},
			want: `{"id":"16","data":{"error":{"message":"Address exceeds the 256-byte limit"}}}`,
		},
	}
...
func Test_PublicServer_BitcoinType(t *testing.T) {
	...
	t.Run("concurrent xpub requests", func(t *testing.T) {
		const requests = 8
		...
		for i := 0; i < requests; i++ {
			...
			go func() {
				defer wg.Done()
				<-start
				resp, err := http.Get(ts.URL + "/api/v2/xpub/" + dbtestdata.Xpub + "?details=txs")
				if err != nil {
					errs <- err
					return
				}
